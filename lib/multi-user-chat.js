...
MultiUserChat.prototype.registerEvents = function() {
    ...
    this.socket.on('xmpp.muc.subject', function(data, callback) {
        self.setSubject(data, callback)
    })
    this.socket.on('xmpp.muc.destroy', function(data, callback) {
        self.destroyRoom(data, callback)
    })
}
...
MultiUserChat.prototype.handle = function(stanza) {
    if (stanza.is('message')) {
        ...
        if (!!stanza.getChild('x', this.NS_USER))
