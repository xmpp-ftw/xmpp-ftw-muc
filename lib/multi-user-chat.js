...
MultiUserChat.prototype.registerEvents = function() {
    ...
    this.socket.on('xmpp.muc.role.set', function(data, callback) {
        self.setRole(data, callback)
    })
    this.socket.on('xmpp.muc.role.get', function(data, callback) {
        self.getRole(data, callback)
    })
    ...
    this.socket.on('xmpp.muc.register.info', function(data, callback) {
        self.register(data, callback, 'get')
    })
    this.socket.on('xmpp.muc.register', function(data, callback) {
        self.register(data, callback, 'set')
    })
...
MultiUserChat.prototype.handle = function(stanza) {
    if (stanza.is('message')) {
        ...
        if (null != stanza.getChild('x', this.NS_USER))
...
MultiUserChat.prototype.sendMessage = function(data) {
    ...
    return xhtmlIm.builder(data, {to: to, type: type}, this)
}
...
MultiUserChat.prototype.register = function(data, callback, type) {
    ...
    this.manager.trackId(stanza.root().attr('id'), function(stanza) {
        if (typeof(callback) != 'function')
            return console.error('No callback provided')
        if (stanza.attrs.type == 'error')
            return callback(self._parseError(stanza), null)
        ...
        if (null != (registered = query.getChild('registered'))) {
            data.registered = true
            if (null != (nick = query.getChild('username')))
                data.nick = nick.getText()
            return callback(null, data)
        }
        if (null != (title = query.getChild('title')))
