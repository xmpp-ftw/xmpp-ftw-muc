...
var builder   = require('ltx')
  ...
  , delay     = require('xmpp-ftw').utils['xep-0203']
...
MultiUserChat.prototype.NS_DISCO_INFO = 'http://jabber.org/protocol/disco#info'

MultiUserChat.prototype.X_ROOMUSER_ITEM = 'x-roomuser-item'
...
MultiUserChat.prototype._events = {
    ...
    'xmpp.muc.room.nick': 'getNickname',
    ...
    'xmpp.muc.invite.decline': 'declineInvitation'
}
...
MultiUserChat.prototype.handles = function(stanza) {
    ...
    if (stanza.is('message') &&
        ...
        (x.getChild('invite') || x.getChild('decline'))) return true
    return false
}
...
MultiUserChat.prototype.handle = function(stanza) {
    ...
    if (stanza.is('message')) {
        ...
        if (!!(x = stanza.getChild('x', this.NS_USER))) {
            ...
            if (x.getChild('decline')) return this._handleDeclineInvitation(stanza)
...
MultiUserChat.prototype.handleMessage = function(stanza) {
    ...
    this.socket.send('xmpp.muc.message', message)
    return true
}
...
MultiUserChat.prototype._handleRoomStatusUpdate = function(stanza) {
    ...
    this.socket.send('xmpp.muc.room.config', updates)
    return true
}
...
MultiUserChat.prototype._handleInvite = function(stanza) {
    ...
    this.socket.send('xmpp.muc.invite', data)
    return true
}

MultiUserChat.prototype._handleDeclineInvitation = function(stanza) {
    var data = { room: stanza.attrs.from }
    var decline = stanza.getChild('x').getChild('decline')
    data.from = this._getJid(decline.attrs.from)
    var reason;
    if (!!(reason = decline.getChildText('reason')))
        data.reason = reason

    this.socket.send('xmpp.muc.invite.decline', data)
    return true
}
...
MultiUserChat.prototype.handleErrorMessage = function(stanza) {
    ...
    this.socket.send('xmpp.muc.error', error)
    return true
}
...
MultiUserChat.prototype.handlePresence = function(stanza) {
    ...
    var x = stanza.getChild('x', this.NS_USER)
    
    if (x) {
        this._getItemDetails(x, presence)
        this._getStatusValues(x, presence)
        var destroy, reason
        if (!!(destroy = x.getChild('destroy'))) {
    ...
    this.socket.send(event, presence)
    return true
}

MultiUserChat.prototype._getItemDetails = function(x, presence) {
...
MultiUserChat.prototype._getStatusValues = function(x, presence) {
    var statusUpdates = x.getChildren('status')
    if (0 === statusUpdates.length) return
    presence.status = []
...
MultiUserChat.prototype.sendMessage = function(data) {
    ...
    if (-1 === this.rooms.indexOf(data.room))
...
MultiUserChat.prototype.join = function(data) {
    ...
    var stanza = new builder.Element('presence', { to: data.room + '/' + data.nick })
        .c('x', { xmlns: this.NS })
    if (data.history) {
        var history = stanza.c('history')
        if (data.history.maxchars) history.attrs.maxchars = data.history.maxchars
        if (data.history.maxstanzas) history.attrs.maxstanzas = data.history.maxstanzas
        if (data.history.seconds) history.attrs.seconds = data.history.seconds
        if (data.history.since) history.attrs.since = data.history.since
    }
    if (data.password) {
        stanza.c('password').t(data.password)
    }
    this.client.send(stanza)
}
...
MultiUserChat.prototype.create = function(data) {
    if (!data.room)
        return this._clientError('Missing \'room\' key', data)
    if (!data.nick)
        return this._clientError('Missing \'nick\' key', data)
    this.rooms.push(data.room)
    var stanza = new builder.Element('presence', { to: data.room + '/' + data.nick })
    var attributes = { xmlns: this.NS }
    if (data.instant) {
        attributes = { xmlns: dataForm.NS, type: 'submit' }
    }
    stanza.c('x', attributes)
    this.client.send(stanza)
}
...
MultiUserChat.prototype.getNickname = function(data, callback) {
    if (typeof callback !== 'function')
        return this._clientError('Missing callback', data)
    if (!data.room)
        return this._clientError('Missing \'room\' key', data, callback)
    var stanza = new builder.Element(
        'iq',
        { type: 'get', id: this._getId(), to: data.room }
    ).c('query', { xmlns: this.NS_DISCO_INFO, node: this.X_ROOMUSER_ITEM })

    var self = this
    this.manager.trackId(stanza, function(stanza) {
        if ('error' === stanza.attrs.type)
            return callback(self._parseError(stanza))
        var identity = stanza.getChild('query').getChild('identity')
        if (!identity)
            return callback({ type: 'cancel', condition: 'item-not-found' })

        callback(null, { nick: identity.attrs.name })
    })
    this.client.send(stanza)
}
...
MultiUserChat.prototype.declineInvitation = function(data){
    if (!data.room)
        return this._clientError('Missing \'room\' key', data)
    if (!data.to)
        return this._clientError('Missing \'to\' key', data)
